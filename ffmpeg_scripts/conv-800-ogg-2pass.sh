...
time (
    ...
    #PASS 1
    time ffmpeg -i "$origfn" -codec:v libtheora -b:v 500k -vf scale=800:-1 -an -pass 1 -f ogg -y /dev/null

    #PASS 2
    time ffmpeg -i "$origfn" -codec:v libtheora -b:v 500k -vf scale=800:-1 -codec:a libvorbis -b:a 128k -pass 2 -f ogg -y "$newfn"
